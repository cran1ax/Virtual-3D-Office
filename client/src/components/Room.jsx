...
import { checkProximityToDeskComputer } from "../utils/proximityDetection";
...
export const Room = () => {
  ...
  const [lastProximityState, setLastProximityState] = useState(false);
  ...
  // Add proximity detection effect
  useEffect(() => {
    const checkProximity = () => {
      const character = scene.getObjectByName(`character-${user}`);
      if (!character) {
        console.log('Character not found for proximity check');
        return;
      }

      // Convert character position to grid coordinates
      const characterGridPos = vector3ToGrid(character.position);
      console.log('Character grid position:', characterGridPos);
      
      // Find all desk computers
      const deskComputers = items.filter(item => item.name === 'deskComputer');
      if (deskComputers.length === 0) {
        console.log('No desk computers found in items');
        return;
      }

      // Check proximity to any desk computer
      const isNearAnyDesk = deskComputers.some(deskComputer => {
        const dx = characterGridPos[0] - deskComputer.gridPosition[0];
        const dz = characterGridPos[1] - deskComputer.gridPosition[1];
        const distance = Math.sqrt(dx * dx + dz * dz);
        console.log(`Distance to desk computer at ${deskComputer.gridPosition}:`, distance);
        return distance <= 2; // 2 grid units threshold
      });
      
      console.log('Is near any desk computer:', isNearAnyDesk);
      
      // Only emit if the proximity state has changed
      if (isNearAnyDesk !== lastProximityState) {
        console.log('Proximity state changed:', isNearAnyDesk);
        socket.emit('nearDeskComputer', isNearAnyDesk);
        setLastProximityState(isNearAnyDesk);
      }
    };

    // Check proximity every 500ms for more responsive updates
    const interval = setInterval(checkProximity, 500);
    return () => clearInterval(interval);
  }, [scene, user, items, lastProximityState, vector3ToGrid]);
