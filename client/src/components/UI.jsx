...
export const UI = () => {
  ...
  const [nearDeskComputer, setNearDeskComputer] = useState(false);
  ...
  useEffect(() => {
    socket.on('nearDeskComputer', (near) => {
      console.log('Received nearDeskComputer event:', near);
      setNearDeskComputer(near);
      if (!near) {
        setShowLaptopUI(false);
      }
    });
    socket.on('userRole', (role) => {
      console.log('Received userRole event:', role);
      setIsAdmin(role.isAdmin);
    });
    return () => {
      socket.off('nearDeskComputer');
      socket.off('userRole');
    };
  }, []);
  ...
  }, [nearDeskComputer, isAdmin, showLaptopUI, roomID, shopMode, buildMode]);
  ...
  return (
    <>
      ...
      >
        {showLaptopUI && (
          <LaptopUI
            position={{ x: window.innerWidth / 2 - 480, y: window.innerHeight / 2 - 300 }}
            size={{ width: 960, height: 600 }}
            onClose={() => setShowLaptopUI(false)}
            isAdmin={isAdmin}
            socket={socket}
          />
        )}
        ...
        <div className="fixed inset-4 flex items-center justify-end flex-col pointer-events-none select-none">
          ...
          <div className="flex items-center space-x-4 pointer-events-auto">
            ...
            {nearDeskComputer && !shopMode && !buildMode && (
              <button
                className="p-4 rounded-full bg-slate-500 text-white drop-shadow-md cursor-pointer hover:bg-slate-800 transition-colors"
                onClick={() => {
                  console.log('Opening laptop UI');
